...
async function readSheetQuestions(sheetId: string, projectId: string): Promise<Question[]> {
  ...
  return sheet.rows.map((row) => {
    ...
    return {
      ...
      status: (cell("Status") as "open" | "replied") || "open",
...
async function addSheetQuestion(
  sheetId: string,
  q: Omit<Question, "id" | "createdAt" | "status">,
  existingId?: string,
...
export async function replyToQuestion(questionId: string, reply: string, repliedBy: string): Promise<Question | null> {
  ...
  if (q) {
    q.reply = reply;
    q.repliedBy = repliedBy;
    q.repliedAt = new Date().toISOString();
    q.status = "replied";
    saveJsonQuestions(questions);

    // Fire-and-forget Smartsheet update
    const config = getSmartsheetConfig(q.projectId);
    if (config.questionsSheetId) {
      replyOnSheet(config.questionsSheetId, questionId, reply, repliedBy).catch((e) =>
  ...
  for (const project of projects) {
    ...
    try {
      const result = await replyOnSheet(config.questionsSheetId, questionId, reply, repliedBy);
      if (result) {
