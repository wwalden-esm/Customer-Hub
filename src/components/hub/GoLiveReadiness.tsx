    ...
    <>
      <div
        className={`rounded-full flex items-center justify-center shrink-0 ${
          item.done ? "bg-emerald-500" : "border-2 border-esm-border"
        }`}
      ...
      <span className={`text-sm ${item.done ? "text-esm-muted line-through" : "text-esm-black"}`}>
        {item.label}
      </span>
  ...
  return (
    <section className="bg-white border border-esm-border rounded-card p-5" aria-labelledby="readiness-heading">
      ...
      {daysToGoLive !== null && daysToGoLive <= 60 && (
        <div className={`text-xs font-medium mb-3 px-3 py-1.5 rounded-card ${
